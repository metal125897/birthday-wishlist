...
Перед каждым коммитом выполнить `npm run snapshot:data`. Команда читает текущие публичные данные Firestore и обновляет `data/firestore-snapshot.json`; этот файл нужно коммитить вместе с кодом. Благодаря этому GitHub хранит версии не только интерфейса, но и фактического списка подарков. `npm run check` проверяет целостность снимка, а не подменяет им рабочую базу.
...
4. Workflow `.github/workflows/pages.yml` сначала выполняет `npm run check` и только после успешной проверки публикует папку `dist/`.
...
Перед локальной публикацией использовать `npm run check`: это тот же минимальный CI-шлюз, который GitHub Actions запускает перед развёртыванием.
